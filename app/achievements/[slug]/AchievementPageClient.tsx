...
import { Calendar, ArrowLeft, ExternalLink, Trophy, Users } from "lucide-react";
...
export default function AchievementPageClient({ slug }: { slug: string }) {
  const [achievement, setAchievement] = useState<AchievementRecord | null>(null);
  ...
  if (isLoading) {
    return (
      <main className="min-h-screen bg-ers-black text-white font-body selection:bg-ers-yellow selection:text-black flex items-center justify-center">
        <div className="text-gray-500 font-mono">Loading achievement...</div>
      </main>
    );
  }
  ...
  if (!achievement) {
    return (
      <main className="min-h-screen bg-ers-black text-white font-body selection:bg-ers-yellow selection:text-black flex flex-col items-center justify-center gap-6 px-6 text-center">
        <h1 className="text-2xl md:text-4xl font-tech text-ers-yellow">Achievement not found</h1>
        <Link href="/achievements" className="text-ers-yellow hover:text-white transition-colors font-mono">
